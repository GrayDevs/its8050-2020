...
def _get_temperature(sense_hat: SenseHat, fahrenheit=False):
	temperature = sense_hat.get_temperature()  # By default in Celcius
...
def _get_humidity(sense_hat):
	humidity = sense_hat.get_humidity()
	return round(humidity, 1)


def _get_pressure(sense_hat):
...
def client_loop(client, sense_hat):
	# Client Loop
	client.loop_start()
	try:
		while True:
			t = _get_temperature(sense_hat, False)
			h = _get_humidity(sense_hat)
			p = _get_pressure(sense_hat)
	...
	sense_hat.clear()  # clean matrix upon exit
	pass


def main():
	# 1. init sense_hat
	sense = SenseHat()  # instantiate a sense_hat object
	sense.clear()  # clear the LED matrix (turn off all lights)

	# 2. init MQTT
	client = init_mqtt()

	# 3. enter MQTT loop
	client_loop(client, sense)
	exit(0)
